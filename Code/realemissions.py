...
Errormax=oneboxRK4(k,Errormax,C0,dt,MW)*1.07
Errormin=oneboxRK4(k,Errormin,C0,dt,MW)*1.07
...
plt.plot(timebar, oneboxRK4(k,P,C0,dt,MW)*1.07,label='RK4 model')
...
plt.plot(timebar, oneboxeulerfw(k,P,C0,dt,MW)*1.07,label='Euler model')
plt.plot(timebar, oneboxRK4(k,P,C0,dt,MW)*1.07,label='RK4 model')
plt.plot(O_df['time'],O_df[substance]/(10**12),label='AGAGE observations')
plt.ticklabel_format(axis='y', scilimits=(-12,-12))
plt.ylabel(substance+' concentration [ppt]')
